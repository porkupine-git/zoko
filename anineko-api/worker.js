...
async function verifyTurnstileToken(request, env, ctx) {
    ...
    if (clientIp) {
        ...
        if (env?.ANINEKO_CACHE || env?.ANIKO_CACHE) {
            try {
                const cacheBinding = env.ANINEKO_CACHE || env.ANIKO_CACHE;
...
export default {
    async fetch(request, env, ctx) {
        ...
        try {
            // 0. Root / API Index: Redirect to main website to eliminate open documentation & endpoint leaks
